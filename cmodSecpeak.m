function [class1,class0] = cmodSecpeak(profiles_matrix,t_start,visual_en,shft_l,shft_r,wfilter,ksec,t_basal_0,t_basal_n)
%%%%%%%%%%%%%%%%%%%%%%%%%% Vars & Params %%%%%%%%%%%%%%%%%%%%%%%%%%
...
f = waitbar(0,'Cmod secondary peaks progress...');
for i_prof = 1:n_prof  % Main for loop across profiles        
    ...
    waitbar(i_prof/n_prof,f,'Cmod secondary peaks progress...');
end
close(f)
...
if visual_en
    ...
    title("Class no secondary peaks")
    ylim([min(min(ca_prof_smooth)) max(max(ca_prof_smooth))])

    fclass1=figure;
    plot(t,ca_prof_smooth(class1,:)); hold on
    stem(t_start,max(max(ca_prof_smooth)),'r')
    title("Class with secondary peaks")
    ylim([min(min(ca_prof_smooth)) max(max(ca_prof_smooth))])
end
