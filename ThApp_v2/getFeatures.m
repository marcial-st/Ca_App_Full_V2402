...
if ~isempty(spk_u)
    ...
    for k=1:1:length(actividades)
        ...
        if ((widx ~= 1)&&(widx ~= length(aux)))
            ...
            if (aux(widx)+epsilon)>length(ca_prof_smooth) 
                prom_l(k) = max(ca_prof_smooth(aux(widx-1):end))-ca_prof_smooth(aux(widx-1));
                prom_r(k) = max(ca_prof_smooth(aux(widx-1):end))-ca_prof_smooth(aux(widx+1));
                prom_max(k) = max(ca_prof_smooth(aux(widx-1):end))-max([ca_prof_smooth(aux(widx+1)),ca_prof_smooth(aux(widx-1))]);
                wact(k) = aux(widx+1)-aux(widx-1);
                spk_u(k,3) = find(ca_prof_smooth== max(ca_prof_smooth(aux(widx-1):end)))*3;
                spk_u(k,4) = max(ca_prof_smooth(aux(widx-1):end));
            else
    ...
    disp('getFeatures WRN Empty spk_u')
    prom_l=[];
    prom_r=[];
    prom_max=[];
    wact=[];
