function [class0,class1,class2,class3] = cmod_Basal_Response(profiles_matrix,t_start,visual_en,shft_l,shft_r,wfilter,ksec,t_basal_0,t_basal_n)
%%%%%%%%%%%%%%%%%%%%%%%%%% Vars & Params %%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     [pks,pks_loc,pks_w,pks_prom] = findpeaks(ca_prof_smooth(i_prof,shft_l:end-shft_r),'MinPeakProminence',pk_h_th,'WidthReference','halfprom');
    ...
    pk_h_th = 0.024;
    i_prof
    [pks,pks_loc,pks_w,pks_prom] = findpeaks_wrapper(ca_prof_smooth(i_prof,shft_l:end-shft_r));
...
%         pks = pks(pks_w>=pk_w_th);
        pks = pks(pks_prom>=pk_h_th);
        if isempty(pks)
            class0(i_c0,1) = i_prof;
            i_c0 = i_c0+1;
        else
            pks_loc = pks_loc+shft_l-1;
            pks_loc = pks_loc(and((pks_w>=pk_w_th),(pks_prom>=pk_h_th)));
