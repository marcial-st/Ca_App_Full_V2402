function [class1,class0] = cmodMainpeak(profiles_matrix,t_start,w_mainpk,visual_en,shft_l,shft_r,wfilter,kmain,t_basal_0,t_basal_n)
%%%%%%%%%%%%%%%%%%%%%%%%%% Vars & Params %%%%%%%%%%%%%%%%%%%%%%%%%%
...
f = waitbar(0,'Cmod main peak progress...');
for i_prof = 1:n_prof  % Main for loop across profiles    

    pk_h_th = kmain*nanstd(noise_prof(i_prof,t_basal_0:t_basal_n));
    [pks,pks_loc,pks_w] = findpeaks(ca_prof_smooth(i_prof,shft_l:end-shft_r),'MinPeakProminence',pk_h_th,'WidthReference','halfprom');        
    if ~(isempty(pks_loc))
        pks_loc = pks_loc+shft_l-1;
        pks_loc = pks_loc(pks_w>=pk_w_th);        
        pks_loc = pks_loc(t_start<=pks_loc);
        pks_loc = pks_loc(pks_loc<=(t_start+w_mainpk));
        if isempty(pks_loc)
...
%         pause(0.2)          
%     end
    waitbar(i_prof/n_prof,f,'Cmod main peak progress...');
end
close(f)
...
if visual_en
    ...
    title("Class no main peak")
    ylim([min(min(ca_prof_smooth)) max(max(ca_prof_smooth))])

    fclass1=figure;
    plot(t,ca_prof_smooth(class1,:)); hold on
    stem(t_start,max(max(ca_prof_smooth)),'r')
    title("Class with main peaks")
    ylim([min(min(ca_prof_smooth)) max(max(ca_prof_smooth))])
end
