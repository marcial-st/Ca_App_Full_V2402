 function [pks_all,pk_max_loc_f,pks_w_all,pks_prom_all,tri_vector] = findpeaks_wrapper(profile_smooth,visual_en,i_prof)
Ts = 3;
min_sep = 0;
...
for k=1:1:length(pk_max_loc_f)      
    ...
    if ~isempty(aux_min)
        ...
        tri_vector(pk_min_loc(aux_min(end))) = -1;
        pks_w_all(k) = (pk_max_loc_f(k)-pk_min_loc(aux_min(end)))*Ts;
        if visual_en
            colorm = (0.85-0.25).*rand(1,3) + 0.25;
            plot((pk_max_loc_f(k)-1)*Ts,profile_smooth(pk_max_loc_f(k)),'Marker','x','MarkerEdgeColor',colorm)
            plot((pk_min_loc(aux_min(end))-1)*Ts,profile_smooth(pk_min_loc(aux_min(end))),'Marker','+','MarkerEdgeColor',colorm)
    ...
    elseif(k == 1)
        ...
        if visual_en
            colorm = (0.85-0.25).*rand(1,3) + 0.25;
            plot((pk_max_loc_f(k)-1)*Ts,profile_smooth(pk_max_loc_f(k)),'Marker','x','MarkerEdgeColor',colorm)
            plot(Ts,profile_smooth(1),'Marker','+','MarkerEdgeColor',colorm)
            text((pk_max_loc_f(k)-1)*Ts,profile_smooth(pk_max_loc_f(k))+0.01,num2str(pks_prom_all(k),'%0.5f'),'Color',colorm,'FontSize',10,'Rotation',90);
        end
    else
